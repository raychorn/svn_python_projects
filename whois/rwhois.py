...
class DomainRecord(WhoisRecord):
    ...
    parsemap={ 'whois.networksolutions.com' : 'ParseWhois_NetworkSolutions' ,
               ...
               'whois.godaddy.com'          : 'ParseWhois_GoDaddyCOM'
               }
    ...
    def __str__(self):
        try:
            ...
            for k,v in self.data.iteritems():
                items.append(k)
                items.append(v if (isinstance(v,str)) else '\t'+'\n\t'.join(v))
            return '\n'.join(items)
    ...
    def _ParseWhois(self):
        ...
        if parser==None:
            raise AttributeError('NoParser for %s' % self.whoisserver)
    ...
    ## ----------------------------------------------------------------------
    ##
    def ParseWhois_GoDaddyCOM(self):
        from vyperlogix.iterators import iterutils
        
        parts = re.split("((?:(?:Administrative Contact|Registrant|Technical Contact|Registered through|Domain servers in listed order),?[ ]*)+:)", self.page)
        self.data['INFO'] = parts[0]
        t = [tuple(pair) for pair in iterutils.itergroup(parts[1:],2)]
        d = dict(t)
        for k,v in d.iteritems():
            self.data[k] = [t.strip() for t in v.split('\r\n') if (len(t.strip()) > 0)]
        self.registrant = self.contacts = None # mark this as a special record
