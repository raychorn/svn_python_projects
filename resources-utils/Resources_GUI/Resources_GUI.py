...
from vyperlogix.pyax_utils.hostify import hostify
...
__version__ = '1.0.0.0'
s_productName = 'Resources Utility'
...
class MainFrame(wx.App):
    def OnInit(self):
	import Resources_GUI_Dialog
	self.__child_frame = Resources_GUI_Dialog.Dialog(None, title=__productName__, progress_title='Processing')
        self.__child_frame.Center(wx.BOTH)
        self.__child_frame.Show(True)
        self.SetTopWindow(self.__child_frame)
	
	self.__log__ = wxLog(self.__child_frame.textboxLog)
    ...
    def doProcessing(self,frame,gauge):
	from vyperlogix.misc import ObjectTypeName
	n = 10
	try:
	    if (n > 0):
		i = 1
		frame.number = n
		print >>frame.log, '%s :: frame.number is "%s".' % (ObjectTypeName.objectSignature(self),frame.number)
		for i in xrange(0,frame.number+1):
		    frame.count = i
		    gauge.number = 100
		    for x in xrange(0,105,5):
			gauge.count = x
			if (self.__isAppClosed):
			    break
			wx.MilliSleep(1000)
		    if (self.__isAppClosed):
			break
	    ...
	    wx_PopUp_Dialog(parent=self.__child_frame,msg=_details,title='ERROR',styles=wx.ICON_ERROR)
	...
	try:
	    ...
	    self.doProcessing(self,self.__child_frame.gauge_panel)
	    self.startTimer()
	except Exception, details:
	    _details = _utils.formattedException(details)
	    wx_PopUp_Dialog(parent=self.__child_frame,msg=_details,title='ERROR',styles=wx.ICON_ERROR)
