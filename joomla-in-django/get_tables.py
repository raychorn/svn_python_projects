...
if (_cname == 'web22.webfaction.com'):
    ...
    conn_str = 'mysql://root:peekaboo@localhost:3306/nearbyin_jo152'
...
if (__name__ == '__main__'):
    tables = [table for table in agent.table_names if (table.startswith('jos_'))]
    s_cmd = _cmd.replace(_symbol_table_names,','.join(tables)).replace(_symbol_file_name,'jos_tables.py')
    io_buf = StringIO()
