...
__to_hex__ = lambda num:'%08X' % (num if (_utils.__is_numerical__(num)) else 0)
...
class CompDB:
    ...
    def set_doCRC32(self, true_or_false):
        self.doCRC32 = true_or_false
        
    def set_noComments(self, true_or_false):
        self.noComments = true_or_false
    ...
    def analyse_file(self, target=False, table_prefix='plays_'):
        ...
        if (self.noComments):
            _utils._noSQLComments(file_name)
            pass # add the code to remove sql comments to _utils similar to crc32...
        if (self.doCRC32):
            self.results_CRC32 = {}
            self.results_CRC32[file_name] = __to_hex__(_utils._crc32(file_name)) if (os.path.exists(file_name)) else "0"

            pPretty = PrettyPrint.PrettyPrint('CRC32 Report',['%s :: %s'%(k,v) for k,v in self.results_CRC32.iteritems()],True,' ... ')
            pPretty.pprint()
            print
...
Prints all the differences between 2 DB schemas on the standard output.
...
Options:
  ...
  -c               crc32 for each schema - output to stdout.
  ...
  -z               comments removal (produces a backup file for the original).
...
1. Added -z or --nocomments to remove comments.

Version 1.1

1. Added -c or --crc32 to perform CRC32 on both Schema Files.
"""
        if not stay:
            sys.exit(2)
...
def parseCmdLine():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "aczhp:nkK", ["auto", "crc32", "nocomments", "help", "prefix=", "no-removing", "foreign-key", "no-foreign-key"])           
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"): usage()
            ...
            comp.set_doCRC32(True)            
        elif opt in ("-z", "--nocomments"):
            comp.set_noComments(True)            
