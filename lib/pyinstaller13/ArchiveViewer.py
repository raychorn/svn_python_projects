...
def show(nm, arch, _stdout=sys.stderr):
    if type(arch.toc) == type({}):
        print >>_stdout, " Name: (ispkg, pos, len)"
        toc = arch.toc
    else:
        print >>_stdout, " pos, length, uncompressed, iscompressed, type, name"
        toc = arch.toc.data
    pprint.pprint(toc,_stdout)
