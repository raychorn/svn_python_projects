...
class Concordance():
	...
	def synonymSearch(self,token,concordance):
		if ( (isinstance(concordance,dict)) or (isinstance(concordance,PickledHash.PickledHash)) ):
				...
				for slice in _sliceMax:
					...
					if (token[:slice] == _k):
						if (not self.synonyms.has_key(token)):
							...
							bucket = list(self.synonyms[token])
						_k_in_bucket = False
						for b in bucket:
							if ( (len(b) > 0) and (k == b[0]) ):
								_k_in_bucket = True
								break
						if ( (not _k_in_bucket) and (token != k) ):
							_kToken = ''.join([k,token])
							_tokenK = ''.join([token,k])
							if (self.cacheSequenceMatcher.has_key(_kToken)):
								_ratio = self.cacheSequenceMatcher[_kToken]
							elif (self.cacheSequenceMatcher.has_key(_tokenK)):
								_ratio = self.cacheSequenceMatcher[_tokenK]
							else:
								self.cacheSequenceMatcher[_tokenK] = self.cacheSequenceMatcher[_kToken] = _ratio = SequenceMatcher.computeQuickRatio(k,token)
							bucket.append((k,_ratio))
							self.synonyms[token] = bucket
		elif (self.isDebuggingByLevel(DebugLevels.warnings)):
			print >>sys.stderr, '(synonymSearch) :: Invalid concordance was supposed to be of type dict or PickledHash.PickledHash but is of type "%s".' % type(concordance)
	...
	def quickRatioUsing(self,token1,token2):
		...
		if (self.cacheSequenceMatcher.has_key(_token12)):
			...
			self.cacheSequenceMatcher[_token21] = self.cacheSequenceMatcher[_token12] = _ratio = SequenceMatcher.computeQuickRatio(token1,token2)
		return _ratio
	...
	def makeSynonyms(self):
		for token in self.allToks:
			...
			for slice in _sliceMax:
				...
				self.allSynonyms[tSlice].append(token)
