...
if (__name__ == '__main__'):
	...
	#files = globalVars.sourceFiles('#express_metrix_') # all the files...
	files = globalVars.sourceFiles('#express_metrix_') # just those files that have the keyword in their name
	for f in files:
		#if (f.find('#unknown_packages.txt') > -1):
		d = ConcordanceProcessor.DataProcessor(f, globalVars.pathName(), None, ConcordanceProcessor.DebugLevels(ConcordanceProcessor.DebugLevels.no_debugging))
		#c = ConcordanceProcessor.Concordance(f, None, ConcordanceProcessor.DebugLevels(ConcordanceProcessor.DebugLevels.no_debugging))
	
	if (1):
		r = ConcordanceProcessor.DataReporter()

	if (1):
		for f in globalVars.dataFiles():
			_f = os.sep.join([globalVars.__pathName,f])
			if (os.path.exists(_f)):
				tName = os.sep.join([globalVars.__pathName,'.'.join([f.split('.')[0],'txt'])])
				fHand = open(tName,'w')
				pp = pprint.PrettyPrinter(indent=4,width=80)
				dbx = PickledHash.PickledHash(_f)
				print >>fHand, '(%s) :: (%s) :: dbx=(%s)' % (__name__,_f,str(dbx))
				if (isDebugging):
					for k in dbx.keys():
						v = dbx[k]
