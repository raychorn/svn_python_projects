...
if (__name__ == '__main__'):
	...
	for f in globalVars.reversed_files:
		if (len(f) == 4):
			if (os.path.exists(f[0])):
				dbx = PickledHash.PickledHash(f[0])
				dbx2 = PickledHash.PickledHash(f[1])
				for k,v in dbx.iteritems():
					for t in v:
						dbx2[t] = k
				dbx.close()
			...
			if (os.path.exists(f[2])):
				dbx = PickledHash.PickledHash(f[2])
				dbx2 = PickledHash.PickledHash(f[3])
				for k,v in dbx.iteritems():
					for t in v:
						if (not dbx2.has_key(t)):
							dbx2[t] = [k]
						else:
							x = dbx2[t]
							x.append(k)
