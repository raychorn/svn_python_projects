...
from pyinstaller13 import mf, archive, iu, carchive, bindepend
import pprint, time, py_compile, tempfile
import sets
...
isRunningBuild = (__name__ == '__main__')
if (isRunningBuild):
    try:
...
class PYZ(Target):
    ...
    def assemble(self):
        ...
        toc = list(sets.Set(self.toc).difference(sets.Set(config['PYZ_dependencies'])))
        for (nm, fnm, typ) in toc:
