...
__proxies__ = {
    'http': 'http://proxy.compaq.com:8080',
    'https': 'http://proxy.compaq.com:8080'
}

def DownloadFile(url):
    import requests
    local_filename = url.split('/')[-1]
    r = requests.get(url, proxies=__proxies__)
...
def unpack_ant(interface):
    if os.path.exists("apache-ant"): #  or (os.path.exists(os.environ['ANT_HOME']))
