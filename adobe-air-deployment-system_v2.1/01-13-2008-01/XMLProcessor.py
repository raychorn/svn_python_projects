...
def listPackagesAndArchivesRelativeTo(connHandle,top,countOnly=False):
    _files = []
    _xml = listItemToXML(top,'<list-head>')
    connHandle.server.__send__(connHandle,_xml)
    for root, dirs, files in os.walk(top, topdown=True):
	#print >>sys.stderr, '(listPackagesAndArchivesRelativeTo).1 :: root=(%s), files=(%s)' % (root,str(files))
	for name in files:
	    if ( (name.lower().find('.exe') > -1) or (name.lower().find('.pkg') > -1) ):
...
class XMLProcessor:
	...
	_files = (db['PKG'] if db.has_key('PKG') else '').split(',')
	_filesDirty = False
	_filesToRemove = []
	print >>sys.stderr, '(readPersistentPackagesList).(%s) :: _files=(%s)' % (self.persistDbName,str(_files))
	_fDict = {}
	for f in _files:
		...
		_filesDirty = True
		_filesToRemove.append(f)
	print >>sys.stderr, '(readPersistentPackagesList).(%s) :: _filesToRemove=(%s)' % (self.persistDbName,str(_filesToRemove))
	for f in _filesToRemove:
	    i = _files.index(f)
	    if (i > -1):
		_files.remove(f)
	if (_filesDirty):
	    db['PKG'] = ','.join(_fDict.keys())
	db.close()
	return _files
	...
	if (isinstance(_files,list)):
	    print >>sys.stderr, '(writePersistentPackagesList).(%s) :: _files=(%s)' % (self.persistDbName,_files)
	    _fDict = {}
	    for f in _files:
		if (_fDict.has_key(f) == False):
		    _fDict[f] = f
	    ...
	    print >>sys.stderr, '(writePersistentPackagesList).WARNING :: _files is an instance of "%s" but it needs to be an instance of "list".' % (type(_files))
	db.close()
	...
	try:
		...
		for doc in docs:
			...
			if (Commands(_cmd) == Commands(Commands.SHUTDOWN)):
				...
				_path = os.path.abspath(os.curdir if len(argVal) == 0 else argVal[0])
				...
				files = listPackagesAndArchivesRelativeTo(connHandle,_path,_countOnly)
				_files = self.readPersistentPackagesList()
				for f in _files:
				    if (not f in files):
					files.append(f)
				files.sort()
				print >>sys.stderr, '\tfiles=(%s)' % str(files)
				_xml = '<null></null>'
				if (_countOnly):
