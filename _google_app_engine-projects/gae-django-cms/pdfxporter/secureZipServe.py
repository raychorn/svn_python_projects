...
class ZipHandler(webapp.RequestHandler):
  ...
  def get(self, prefix, name):
    ...
    referer = '' if (not hasKey) else self.request.environ['HTTP_REFERER']
    host = self.request.environ['HTTP_HOST']
    host = host.replace('127.0.0.1','localhost') if (referer.find('localhost') > -1) else host
    isOkay = True if (not hasKey) or ( (hasKey) and (referer.lower().find(host.lower()) > -1) ) else False
