...
from g import current_air_sender, current_air_site, queue_email, current_site
...
class RegistrationManager(models.Manager):
    ...
    def send_activation_email(self,new_user,registration_profile,air_id='',domain_override=''):
        ...
        try:
            if (settings.IS_PRODUCTION_SERVER):
                logging.log(logging.INFO,'sender="%s Support <%s>"' % (current_air_sender(air_id),current_air_sender(air_id)))
		if (settings.QUEUE_EMAILS):
		    queue_email(current_air_sender(air_id),new_user.email,"Your User Account Activation for %s" % (current_air_site(air_id)),message,message_html,air_id,settings.SUB_DOMAIN_NAME,is_html=True)
		else:
		    from google.appengine.api import mail
		    mail.send_mail(sender="%s Support <%s>" % (current_air_sender(air_id),current_air_sender(air_id)),
			          to=new_user.email,
			          subject="Your User Account Activation for %s" % (current_air_site(air_id)),
			          body=message,
			          html=message_html)
            else:
        ...
        #try:
                ...
                #logging.log(logging.INFO,'sender="%s Support <%s>"' % (current_air_sender(air_id),current_air_sender(air_id)))
                #mail.send_mail(sender="%s Support <%s>" % (current_air_sender(air_id),current_air_sender(air_id)),
                              #to="%s Support <%s>" % (current_air_sender(air_id),current_air_sender(air_id)),
                              #subject="New User Account Activation for %s (%s)" % (current_air_site(air_id),new_user.email),
    ...
    def send_passwordChg_email(self,user,old_password,password,air_id='',domain_override=''):
        ...
        try:
            if (settings.IS_PRODUCTION_SERVER):
		if (settings.QUEUE_EMAILS):
		    queue_email(current_air_sender(air_id),user.email,"Password Change Request for %s" % (current_air_site(air_id)),message,message_html,air_id,settings.SUB_DOMAIN_NAME,is_html=True)
		else:
		    from google.appengine.api import mail
		    mail.send_mail(sender="%s Support <%s>" % (current_air_sender(air_id),current_air_sender(air_id)),
			          to=user.email,
			          subject="Password Change Request for %s" % (current_air_site(air_id)),
			          body=message,html=message_html)
            else:
        ...
        #try:
                ...
                #mail.send_mail(sender="%s Support <%s>" % (current_air_sender(air_id),current_air_sender(air_id)),
                              #to="%s Support <%s>" % (current_air_sender(air_id),current_air_sender(air_id)),
                              #subject="Password Change Request for %s (%s)" % (current_air_site(air_id),user.email),
                              #body=message,html=message_html)
            #else:
                #logging.info('RegistrationManager.changePassword.INFO --> to=%s, message=%s' % (user.email,message))
        #except Exception, e:
    ...
    def send_problem_email(self,fromName,email,msg,air_id='',domain_override=''):
        ...
        try:
            if (settings.IS_PRODUCTION_SERVER):
		if (settings.QUEUE_EMAILS):
		    queue_email(email,current_air_sender(air_id),subject,message,message_html,air_id,settings.SUB_DOMAIN_NAME,is_html=True)
		else:
		    from google.appengine.api import mail
		    mail.send_mail(sender="%s <%s>" % (fromName,email),
			          to=current_air_sender(air_id),
			          subject=subject,
			          body=message,html=message_html)
            else:
                logging.info('RegistrationManager.create_inactive_user.INFO --> to=%s, message=%s' % (new_user.email,message))
    ...
    def create_inactive_user(self, username, password, email, first_name, last_name, air_id='', domain_override="", send_email=True):
        ...
        new_user = User(username=username, key_name="key_"+username.lower(), email=email, first_name=first_name, last_name=last_name, is_active=False)
...
####################################################################################
## BEGIN: PDFXporter Models
####################################################################################
class Statement(db.Model):
    user = db.ReferenceProperty(User, verbose_name=_('user'))

class AsciiText(db.Model):
    statement = db.Reference(Statement, verbose_name=_('statement'))
    items = db.ListProperty(str,verbose_name=_('items'))
    num = db.IntegerProperty(verbose_name=_('num'))
    timestamp = db.DateTimeProperty(auto_now=True)

class LineItem(db.Model):
    item = db.StringProperty(verbose_name=_('item'))
    statement = db.Reference(Statement, verbose_name=_('statement'))
    timestamp = db.DateTimeProperty(auto_now=True)

class Element(db.Model):
    _type = db.StringProperty(verbose_name=_('type'))
    matches = db.ListProperty(str,verbose_name=_('matches'))
    item = db.Reference(LineItem, verbose_name=_('item'))
    timestamp = db.DateTimeProperty(auto_now=True)
####################################################################################
## END! PDFXporter Models
####################################################################################
...
class Geolocation(db.Model):
    ...
    altitude = db.FloatProperty(verbose_name=_('altitude'))
    heading = db.FloatProperty(verbose_name=_('heading'))
    horizontalAccuracy = db.FloatProperty(verbose_name=_('horizontal_accuracy'))
    speed = db.FloatProperty(verbose_name=_('speed'))
    _type = db.StringProperty(verbose_name=_('type'))
    verticalAccuracy = db.FloatProperty(verbose_name=_('vertical_accuracy'))
    timestamp = db.DateTimeProperty(auto_now=True)
...
####################################################################################
## BEGIN: Starbucks free-4u Models
####################################################################################
class StarbucksDrawing(db.Model):
    date = db.DateTimeProperty(auto_now=False,verbose_name=_('date'))
    prize = db.FloatProperty(verbose_name=_('prize'))
    timestamp = db.DateTimeProperty(auto_now=True)

class StarbucksCheckIn(db.Model):
    user = db.ReferenceProperty(User, verbose_name=_('user'))
    gps = db.ReferenceProperty(Geolocation, verbose_name=_('gps'))
    version = db.StringProperty(verbose_name=_('version'))
    drawing = db.ReferenceProperty(StarbucksDrawing, verbose_name=_('drawing'))
    timestamp = db.DateTimeProperty(auto_now=True)

class StarbucksWinner(db.Model):
    winner = db.Reference(StarbucksCheckIn, verbose_name=_('winner'))
    drawing = db.Reference(StarbucksDrawing, verbose_name=_('drawing'))
    timestamp = db.DateTimeProperty(auto_now=True)
    
class StarbucksFeedback(db.Model):
    checkin = db.Reference(StarbucksCheckIn, verbose_name=_('checkin'))
    timestamp = db.DateTimeProperty(auto_now=True)
####################################################################################
## END! Starbucks free-4u Models
####################################################################################


####################################################################################
## BEGIN: EMail Processing Models
####################################################################################
class EMailQueue(db.Model):
    uuid = db.StringProperty(verbose_name=_('uuid'))
    email_from = db.StringProperty(verbose_name=_('from'))
    email_to = db.StringProperty(verbose_name=_('to'))
    email_subject = db.StringProperty(verbose_name=_('subject'))
    email_body = db.TextProperty()
    email_altbody = db.TextProperty()
    timestamp = db.DateTimeProperty(auto_now=True)
