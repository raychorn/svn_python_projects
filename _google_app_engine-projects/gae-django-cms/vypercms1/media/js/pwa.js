...
function selectphoto(photoURL,photosize,desr){
	var oImg=document.getElementById('picture');
	//oImg.onclick=function() {
	oImg.onload=function() {
		//click handler for oImg
		//alert(photoURL+"?imgmax="+photosize);
		//parent.myform.src2.value = photoURL+"?imgmax="+photosize ;//ie
		//parent.document.all.src2.innerText = photoURL+"?imgmax="+photosize ;//ie
		//parent.myform.src2.innerText = photoURL+"?imgmax="+photosize ;//ie
		//var condId = parent.document.getElementById("src");   
    //condId.innerText = photoURL+"?imgmax="+photosize ;//ie 
    //condId.value = photoURL+"?imgmax="+photosize ;//ie ff gg
    parent.document.getElementById("src").value = photoURL+"?imgmax="+photosize ;
    parent.document.getElementById("alt").value = desr ;
    parent.document.getElementById("title").value = desr ;
    //
    parent.ImageDialog.showPreviewImage(photoURL+"?imgmax="+photosize);
		}
} 
...
function picasaweb(j){ //returns the list of all albums for the user
 $("<div style='margin-left:3px'> Album </div><!--<div style='text-align:right; margin-right:5px; margin-top:-14px'><a target=PICASA class='standard' href='http://picasaweb.google.com/"+username+"/'>View this gallery in Picasa</a></div>-->");
 $("<table border=0><tr>");
 ...
 // then display them in a table with 4 columns (along with the album title)
  ...
  $("<td valign=top><a class='standard' href='?albumid="+id_base+"'><img src='"+img_base+"?imgmax=72&crop=1' class='pwimages' /></a>");
  $("<table border=0><tr><td></td></tr></table><center><a class='standard' href='?albumid="+id_base+"'>"+ j.feed.entry[i].title.$t +"</a></center></td>");
...
function albums(j){  //returns all photos in a specific album
 ...
 $("<div style='margin-left:3px'><a class='standard' href='" + window.location.protocol + "//" + window.location.hostname+window.location.pathname+"'>Album Home</a> &gt; "+ j.feed.title.$t +"&nbsp;&nbsp;["+np+" item"+item_plural+"]</div><!--<div style='text-align:right; margin-right:5px; margin-top:-14px'><a target=PICASA class='standard' href='"+album_link+"'>View this album in Picasa</a></div>-->");
 $("<table border=0><tr>");
 ...
 for(i=0;i<j.feed.entry.length;i++){
  ...
  $("<td valign=top><a title='hahah' href='"+link_url+"'><img src='"+img_base+"?imgmax=64&crop=1' class='pwimages' /></a>");
...
$("<div style='margin-left:3px'><a class='standard' href='"+ window.location.protocol + "//" + window.location.hostname+window.location.pathname+"'> Album Home </a> &gt; <a class='standard' href='" + album_base_path + "'>" + my_fixed_galleryname + "</a> &gt; <!--" + img_filename + "-->" + current_index_text + "</div><!--<div style='text-align:right; margin-right:3px; margin-top:-14px'><a target=PICASA class='standard' href='"+photo_link+"'>View this image in Picasa</a></div>-->");
...
if (n1 == null) //we're at the last picture in the album; going forward takes us to the album index
 ...
 if (photo_array.length > 1) { $("<td><a class='standard' href='"+prev+"'><img border=0 alt='Previous item' src='/media/images/back.jpg'></a> </td><td></td>"); }
 $("<td> <a class='standard' href='"+album_base_path+"'><img border=0 alt='Back to album index' src='/media/images/home.jpg'></a> </td>");
 if (photo_array.length > 1) { $("<td></td><td> <a class='standard' href='"+next+"'><img border=0 alt='Next item' src='/media/images/next.jpg'></a></td>"); }
 $("</tr></table></center>");
 ...
 $("<center><div border=0 id='myphoto' ><img id='picture' width="+display_width+" src='"+img_base+"?imgmax="+photosize+"' class='pwimages' /></div></center>");
 ...
 selectphoto(img_base,photosize,j.entry.media$group.media$description.$t);
}
