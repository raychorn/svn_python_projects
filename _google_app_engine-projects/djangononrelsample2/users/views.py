...
from g import dict_to_json, today, _admin_username, _admin_password
...
def default(request):
    ...
    c = __context__(request)
    is_not_logged_in = current_user is AnonymousUser
    c['action'] = '/login/' if (is_not_logged_in) else '/logout/'
    c['registeraction'] = '/register/'
    c['expected_username'] = _admin_username
    c['expected_password'] = _admin_password
    c['current_user'] = current_user
    c['IS_NOT_LOGGED_IN'] = is_not_logged_in
    return render_to_response('users.html', c, content_type="text/html")

#@csrf_protect
def login(request):
    '''
    Authentication is being faked here for demo purposes because there are initialization issues - users have to be created and there is no real reason to do this for a static demo.
    Also the requirements did not specify how the authentication was to be done and abiguity favors the one who did not write the document.
    '''
    c = __context__(request)
    username = str(django_utils.get_from_post_or_get(request, 'username', default=''))
    password = str(django_utils.get_from_post_or_get(request, 'password', default=''))
	...
	try:
		...
		is_p = True
	    request.session['__user__'] = {'username':username,'password':password} if (is_u and is_p) else get_user(request)
	except Exception, ex:
	    is_ex = True
	    c['debug'] = str(ex)
	if (not is_ex):
	    c['debug'] = '%s, %s --> %s, %s --> %s, %s --> %s' % (username,password,_admin_username,_admin_password,is_u,is_p,get_user(request))
	current_user = get_user(request)
	c['current_user'] = current_user
	c['IS_NOT_LOGGED_IN'] = current_user is AnonymousUser
    return render_to_response('users.html', c, content_type="text/html")
...
def logout(request):
    ...
    c['current_user'] = current_user
    c['IS_NOT_LOGGED_IN'] = current_user is AnonymousUser
    request.session['__user__'] = current_user
    return render_to_response('users.html', c, content_type="text/html")
...
def register(request):
    ...
    c['cancel'] = '/login'
    return render_to_response('users-register.html', c, content_type="text/html")
...
#@requires_csrf_token
def rest_handle_user_login(request,parms=None,browserAnalysis=None,__air_id__=None,__apiMap__={},data={}):
    d = {}
    u = {}
    _user = get_user(request)
    try:
