...
def create_admin_user(request):
    ...
    _admin_password = 'peekab00'
    user = User.get_by_key_name(_admin_username)
    if not user or user.username != _admin_username or not (user.is_active and
            user.is_staff and user.is_superuser and
            user.check_password(_admin_password)):
        user = User(key_name=_admin_username, username=_admin_username,
            email='raychorn@gmail.com', first_name='Ray', last_name='Horn',
