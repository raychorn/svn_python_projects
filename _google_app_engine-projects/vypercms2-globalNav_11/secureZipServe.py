...
"""
__allowed_domains__ = ['www.raychorn.com']
...
class ZipHandler(webapp.RequestHandler):
  ...
  def get(self, prefix, name):
    ...
    """
    hasKey = self.request.environ.has_key('HTTP_REFERER')
    referer = '' if (not hasKey) else self.request.environ['HTTP_REFERER'].lower()
    host = self.request.environ['HTTP_HOST'].lower()
    isOkay = True if (not hasKey) or ( (hasKey) and (referer.find(host) > -1) ) or ( (hasKey) and (referer in __allowed_domains__) ) else False
    fname = prefix + '.zip/' + name
    _msg = "HTTP_REFERER=%s, HTTP_HOST=%sm isOkay=%s, fname=%s" % (referer, host, isOkay, fname)
    if (isOkay):
      logging.debug('FOUND: %s', _msg)
      self.ServeFromZipFile(prefix + '.zip', name)
    else:
      logging.debug('NOT FOUND: %s', _msg)
      self.error(404)
