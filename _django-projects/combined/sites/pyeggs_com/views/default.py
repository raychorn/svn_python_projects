...
#from django.forms import BaseForm, Form, CharField
...
from models import User, UploadFile, UserActivity, UserContract
...
_title = 'pyEggs&trade; by Vyper Logix Corp, The 21st Century Python Company'
...
def on_successful_registration(form,request):
    ...
    html_context = send_registration_email(email_address, 'pyEggs Online (www.pyeggs.com)', 'http://%s/pyeggs/registration/activate/%s' % (django_utils.get_fully_qualified_http_host(request),keys._encode('%s' % (email_address))), 'http://%s' % (django_utils.get_fully_qualified_http_host(request)))
...
def render_upload_stats(u_stats,num=1,toks=[]):
    aList = []
    if (num == 1):
	aList.append('You are allowed %s uploads per %s day(s).' % (u_stats.contract_num_uploads,u_stats.contract_num_days))
    elif (num == 2):
	aList.append('You have used %s uploads per %s day(s).' % (u_stats.num_uploads,u_stats.num_days))
    elif (num == 3):
	a_link = oohtml.renderAnchor('/upgrade/','Upgrade',target='_top') if (len(toks) == 0) or (toks[0] != 'upgrade') else 'Upgrade'
	aList.append('<NOBR>You are allowed another %2.2f uploads.</NOBR><br/><NOBR>You may wish to %s to get more uploads.</NOBR>' % (u_stats.normal_count,a_link))
    return oohtml.render_table_from_list(aList,num_wide=1)
...
def default(request):
	...
	if (get_is_logged_in(request)):
	    ...
	    anActivity.save()
	    u_stats = zip_upload.get_upload_stats(aUser)
	    u_context = {'UPLOAD_STATS_1':render_upload_stats(u_stats,num=1,toks=url_toks),
			 'UPLOAD_STATS_2':render_upload_stats(u_stats,num=2,toks=url_toks),
			 'UPLOAD_STATS_3':render_upload_stats(u_stats,num=3,toks=url_toks)
			 }
	...
	if (len(url_toks) == 0):
	...
	elif (url_toks[0] == 'login'):
	    if (len(url_toks) == 1):
		_context['LOGIN_REGISTER_BUTTON'] = render_static_html(request,'','_register-button.html',template_folder=_template_folder,context=_default_context) if (not get_is_logged_in(request)) else ''
		resp = login.login(request,onAction='/%s/process/' % (url_toks[0]),onSuccess='/%s/accepted/' % (url_toks[0]))
		...
		if (len(users) > 0):
		    aUser = users[0]
		    if (aUser.password is None) or (aUser.password != request.POST['password']):
			t_content = get_template('_pyeggs_reject_login.html')
	...
	elif ('/'.join(url_toks[0:-1]).lower() == 'pyeggs/registration/activate'):
...
def handle_404(request):
    return HttpResponseNotFound(pages.render_the_template(request,'%s' % (_title),'404_content.html',context={},template_folder=''))
