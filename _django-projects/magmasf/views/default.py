...
def default(request):
    ...
    fname = os.path.join(os.path.dirname(os.path.dirname(__file__)),'logs','%s_logger_%s.txt' % (_title,_utils.timeStampForFileName().split('_')[0]))
    ...
    html = t_content.render(Context({'current_date': now, 'the_title': _title}))
    return HttpResponse(html)
