...
from users.g import queue_email
...
class RegistrationManager(models.Manager):
    ...
    """
    def activate_user(self, username, password, activation_key):
        """
        ...
        if SHA1_RE.search(_activation_key):
            registrations = [p for p in RegistrationProfile.objects.all() if (p.activation_key == activation_key)]
    ...
    def reactivate_user(self,username,password=None,air_id=None,data={},error_symbol=None):
        ...
        if (len(registrations) > 0):
            ...
            if (profile.activation_key == RegistrationProfile.ACTIVATED):
                ...
                profile.save()
                self.send_activation_email(profile.user,password,profile,air_id=air_id,prefix='',reason='Registration',data=data,error_symbol=error_symbol)
            else:
                self.send_activation_email(profile.user,password,profile,air_id=air_id,prefix='Re',reason='Forgot Password',data=data,error_symbol=error_symbol)
            return True
        return False
    
    def decode_activation_key(self,key):
    ...
    def create_inactive_user(self, username, password, email, first_name, last_name, air_id='', domain_override="", send_email=True):
        ...
        if (send_email):
            self.send_activation_email(new_user,password,registration_profile,air_id=air_id,domain_override=domain_override)
