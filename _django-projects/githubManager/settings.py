...
if (DOMAIN_NAME in ['DGLYJ7V1','raychorn-VirtualBox','HORNRA3']):
    DATABASES = {
        'default': {
    ...
    GITHUB_PROXY = '127.0.0.1:9909'
    #MONGODB_ENDPOINT = '127.0.0.1:27017'
    #SLEEPY_MONGOOSE = '127.0.0.1:27080'
    #MONGODB_SERVICE_NAME = 'MongoDB'
    CURRENT_SITE = 'GithubManager.VyperLogix.Com'
    CACHE_LOCATION = [
        '127.0.0.1:11211',
    ]
...
print '6.GITHUB_PROXY=%s' % (GITHUB_PROXY)
if (misc.isString(GITHUB_PROXY)):
    GITHUB_PROXY_TEST = 'http://%s/test' % (GITHUB_PROXY)
    s = _urllib2.get(GITHUB_PROXY_TEST)
    d = json.loads(s) if (misc.isStringValid(s)) else None
    if (not d):
        django_utils.die('7. TERMINATION ERROR: Cannot locate the required GITHUB_PROXY [%s]' % (GITHUB_PROXY))
print '8.GITHUB_PROXY=%s' % (GITHUB_PROXY)
    ...
    LOGGING = {
        ...
        'handlers': {
            'default': {
                ...
                'filename': 'logs/default.log' if (_utils.isUsingWindows) else '/var/log/githubManager/default.log',
            ...
            'request_handler': {
                    'level':'INFO',
                    'class':'logging.handlers.RotatingFileHandler',
                    'filename': 'logs/django_request.log' if (_utils.isUsingWindows) else '/var/log/githubManager/django_request.log',
                    'maxBytes': 1024*1024*5, # 5 MB
                    'backupCount': 5,
                    'formatter':'standard',
            },
