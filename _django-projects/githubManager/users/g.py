...
air_sender = lambda key:__air_sender__.get(key,'githubManager@vyperlogix.com') if (__air_sender__) else None

__air_site__ = {}
__air_site__[symbol_svncloud] = None
air_site = lambda key:__air_site__.get(key,'GitHub Manager @ Vyper Logix Corp.') if (__air_site__) else None
...
class AuthMethods(Enum):
    none = 0
    django = 2**0
    #github = 2**1  # Cannot allow users to login using their Github Accounts because all users MUST be Authenticated by Django.
...
def analyze_and_correct_session_status(request,data):
    from users.views import _error_symbol, _status_symbol
    from users.views import __registration_successful__
    sources = [_error_symbol,_status_symbol]
    vectors = [str(t).find(__registration_successful__) > -1 for t in [request.session.get(s,'') for s in sources]]
    if (any(vectors)):
	vectors = ListWrapper(vectors)
	from users.registration import RegistrationProfile
	registrations = [ r for r in RegistrationProfile.objects.all() if (r.activation_key == RegistrationProfile.ACTIVATED)]

	d_users = {}
	for u in users:
	    regs = [r.user_id for r in registrations if (r.user_id == u.id)]
	    d_users[u.id] = len(regs)
	analysis = [v for v in d_users.values() if (v == 0)]
	if (len(analysis) == 0):
	    i = vectors.findFirstMatching(True)
	    if (i > -1):
		s = sources[i]
		del data[s]
...
def __queue_email__(email_from,email_to,email_subject,email_body,email_body_html,__air_id__,is_html=False,data={},error_symbol=None):
    from django.conf import settings
    if (misc.isStringValid(__EMAIL_POST_ADDRESS__)):
	...
	logging.warn('settings.__EMAIL_POST_ADDRESS__ has not been defined.')
