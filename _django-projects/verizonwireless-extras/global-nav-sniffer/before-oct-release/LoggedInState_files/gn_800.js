gn_iframe = '/imageFiles/Myacct/nos/blank_page.html';
...
function gn_init() {
	...
	var version=parseFloat(temp[1])
		...
		{
			use_ishim=true;
			$('gn').insertAdjacentHTML('beforeEnd', '<iframe src="javascript:false" id="gn_ishim" frameborder="0" scrolling="0" style="position:absolute;top:0;left:0;width:0;height:0;display:none;z-index:498;filter:progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0);"></iframe>');
			var gn = $('gn');
			var lis = $ES('li',gn);
...
// added this to make same as IS look and feel. IS has made this look and feel through JS.
// so, we had to do the same way.
function setButtons()
{		
	...
	if(browser.indexOf('Microsoft') != -1)	// assume IE browser
	{
		var buttons = document.getElementsByTagName('button');
		var buttonsLen = buttons.length;
		for(var i=0; i<buttonsLen; i++)
		{
			if (buttons[i].className == 'searchButton')  {
				//alert('changing button');
				buttons[i].style.width = (buttons[i].getElementsByTagName('span').item(0).offsetWidth+5 ) + 'px';
				//assume that the button has span tag always.				
				buttons[i].getElementsByTagName('span')[0].style.fontSize='1em';
			}
		}	
		
		// changing text box for IE
		var inputs = document.getElementsByTagName('input');
		var inputsLen = inputs.length;
		for(i=0; i<inputsLen; i++) {
			if(inputs[i].id =='q') {
				 //alert('inside input');
				 inputs[i].style.width=(inputs[i].offsetWidth+16)+ 'px';
			}
		}
	}
}
