...
product_codes = {
    '1.0.0'  : "{3ad0c9ed-7e5e-478c-b539-5cc3a2f8772a}"
}
...
def build_database():
    db = msilib.init_database("%s%s.msi" % (package_name,current_version,schema),
                              ProductName="%s %s" % (package_name,current_version),
                              ProductCode=product_codes[current_version],
                              ProductVersion=current_version,
                              Manufacturer=package_author)
...
def add_files(db):
    ...
    dirs={}
    # Add all files in Lib, except lib-tk, test
    pydirs = [(root,"Lib")]
    while pydirs:
