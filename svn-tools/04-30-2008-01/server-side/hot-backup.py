...
svnlook = findUsingPath(r"@SVN_BINDIR@/svnlook") # Path to svnlook utility
...
if (os.path.exists(svnlook)):
  ...
  if (os.path.exists(svnadmin_fname)):
    ...
    svnadmin = findUsingPath(r"@SVN_BINDIR@/svnadmin") # Path to svnadmin utility
...
if (not os.path.exists('.'.join([backup_dir,'zip']))):
  print "Backing up repository to '" + backup_subdir + "'..."
  if have_subprocess:
    err_code = subprocess.call([svnadmin, "hotcopy", repo_dir, backup_subdir, "--clean-logs"])
  else:
