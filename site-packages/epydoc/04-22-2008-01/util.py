...
def collectFilesFrom(fnameSpec):
    _files = []
    pat = os.path.basename(fnameSpec)
    top = fnameSpec.replace(pat,'')
    pat = pat.replace('+','').replace('*','')
    isRecursive = fnameSpec.endswith('+')
    for root, dirs, files in os.walk(top, topdown=True):
        if (root.find('.svn') == -1) and (root.find('_svn') == -1):
            for f in files:
                if (f.find(pat) > -1):
                    _files.append(''.join([root,f]))
        if (not isRecursive):
            break
    return _files
