...
#                     self._last_tag = 'error'
    ...
    def open(self, prjfile):
        from epydoc.docwriter.html_css import STYLESHEETS
        from util import collectFilesFrom
        ...
        try:
            ...
            try:
                for k,v in _args.iteritems():
                    ...
                    if (_k in opts):
                        if (_k == 'modules'):
                            opts['modules'] = tuple(collectFilesFrom(v))
                        else:
                            opts[_k] = v
            except:
                pass
...
def _usage():
    ...
    print '    -prj_url=some_url           specify the URL for the project'
    print '    -target=some_UNC_path       specify the target path for the output'
    print '    -modules=some_file_pattern specify the source modules list using a file path and pattern'
    print
    sys.exit(0)
...
def gui():
    ...
    if (len(_args) > 0) and (len(projects) == 1):
        gui.open(projects[0])
    else:
        gui.mainloop()
