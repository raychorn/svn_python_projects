...
LOG_FILENAME = os.sep.join([os.path.dirname(sys.argv[0]),'phonegapwebservice.log'])
...
logger = logging.getLogger('phonegapwebservice')
...
<html>
<head>
    <title>(c). Copyright 2014, Vyper Logix Corp., All Rights Reserved.</title>
    <style>
        #menu {
            width: 200px;
            float: left;
...
class Index:
    ...
    def GET(self):
        """ Show page """
	s = 'phonegapwebservice %s' % (__version__)
...
class ImageBackend:
	...
	web.header('Content-Type', 'application/json')
	reasons = []
	__image_property__ = 'image'
	data = web.data()
	toks = data.split('=')
	image = toks[-1]
	#image = data[__image_property__] if (data.has_key(__image_property__)) else None
	if (action == 'save'):
	    logger.info('saving image:\n%s' % (image))
	    reasons.append('Saved image.')
...
if __name__ == '__main__':
    '''
    python phonegapwebservice.py 127.0.0.1:9999
    '''
    import re
    ...
    def __init__():
	logger.info('phonegapwebservice %s started !!!' % (__version__))
	flush_handlers()
        app.run()
