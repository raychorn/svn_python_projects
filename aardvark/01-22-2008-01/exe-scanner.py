...
def queueFile(root,f):
    _files_queue.put(filterFolderRoot(root+os.sep+f))

@threadpool(_pool)
def scanFolder(root,_files):
    global _files_queue
    for f in [f for f in _files]:
	if (f.strip().lower().endswith('.exe')):
	    _files_queue.put(filterFolderRoot(root+os.sep+f))

@threadpool(_pool)
def scanFolder2(root,_files):
    global _files_queue
    d = {}
    for f in [f for f in _files]:
    ...
    if (d.has_key('exe')):
	for x in d['exe']:
	    _files_queue.put(filterFolderRoot(root+os.sep+x))

@threadpool(_pool)
def scanFolder3(root,_files):
    d = {}
    for f in [f for f in _files]:
	tname = str(f.split('.')[-1]).strip().lower()
	if (d.has_key(tname) == False):
