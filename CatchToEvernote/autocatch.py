...
__re__ = SmartObject({
    'time':re.compile(r"(?P<year>\d*)\W*(?P<month>\d*)\W*(?P<day>\d*)\s(?P<hours>\d*)\W*(?P<minutes>\d*)", re.MULTILINE)
})
...
def get_zip_file_name(fpath=None):
    folder = "." if (not os.path.exists(fpath)) else fpath
    zip_file_names = ['%s/%s'%(folder,zip_file_name) for zip_file_name in os.listdir(folder) \
...
def get_milliseconds_from_utc(utc):
    try:
        ...
        matches = SmartObject(__re__.time.match(utc).groupdict())
        utc = '%s-%s-%sT%s:%s' % (matches.year,matches.month,matches.day,matches.hours,matches.minutes)
        dt = datetime.datetime.strptime(utc, "%Y-%m-%dT%H:%M")
...
class NoteImporter:
    def __init__(self, token, fpath=None):
        self.fpath = fpath
        self.token = token
        for sandbox in (True, False):
            ...
            try:
                ...
                if not sandbox:
                    ...
                    raise
        zip_file_name = get_zip_file_name(fpath=self.fpath)
    ...
    def create_note(self, notebook_guid, note_file, attachments, title_override=None):
        ...
        try:
            ...
            __title__ = info.xpath("//note/title")[0].text
            __created__ = info.xpath("//note/created")[0].text
            __updated__ = info.xpath("//note/updated")[0].text
            __author__ = info.xpath("//note/note-attributes/author")[0].text
            __content__ = info.xpath("//note/content")[0].text
        except:
            info = data.split('\n')
            __id__ = info[0].split(':')[-1].strip()
            info = info[1:]
            __spaces__ = info[0].split(':')[-1].strip()
            info = info[1:]
            __author__ = info[0].split(':')[-1].strip()
            info = info[1:]
            __updated__ = info[0].split('Modified:')[-1].strip()
            info = info[1:]
            __created__ = info[0].split('Created:')[-1].strip()
            info = info[1:]
            __title__ = info[0]
            info = info[1:]
            __content__ =  '<en-note>%s</en-note>' % ('<![CDATA[%s]]>'%('\n'.join(info)))
        note.notebookGuid = notebook_guid
        note.title = title_override or \
            __title__.replace("\t", " ").strip().encode('utf-8')
        note.created = get_milliseconds_from_utc(__created__)
        note.updated = get_milliseconds_from_utc(__updated__)
        note.attributes = Types.NoteAttributes()
        note.attributes.author = __author__.encode('utf-8')
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        note_importer = NoteImporter(sys.argv[1],fpath=fpath)
