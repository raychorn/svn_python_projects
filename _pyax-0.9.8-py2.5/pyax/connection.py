...
class Connection(object):
    ...
    def __resultToObject(self, result_list):
        ...
        if (len(result_list) and 
            result_list[0] is not None 
...
#            continue
    ...
    is_count_query_re = re.compile("^\s?select\s?count\(\)", re.IGNORECASE)
    def _query_template(self, method_name, query_string, callback, errback):
        ...
        if self.is_count_query_re.match(query_string):
            is_count_query = True
        
        if callback is not None:
            def process_result(result):
                if is_count_query:
                    return self.__process_count_query_result(result, callback)
                else:
                    self.__processAsyncQueryResult(result, callback, errback)

            self._callApex(method, query_string, callback=process_result,
                           errback=errback)
        else:
            query_result = self._callApex(method, query_string)
            if is_count_query:
                return self.__process_count_query_result(query_result)
            else:
    ...
    def __queryMore(self, query_locator):
        return self._callApex(self.svc.queryMore, query_locator)
    ...
    def _unpackQueryResult(self, query_result):
        ...
        records = listify(query_result.get('records'))
    ...
    def __processAsyncQueryResult(self, query_result, callback, errback, records=None):
        ...
        _records, done, query_locator, size = self._unpackQueryResult(query_result)
    ...
    def __processQueryResult(self, query_result):
        ...
        """
        sobject_records, done, query_locator, size = self._unpackQueryResult(query_result)
        while done is False:
            query_more_result = self.__queryMore(query_locator)
            records, done, query_locator, size = self._unpackQueryResult(query_more_result)
            sobject_records.extend(records)

        query_result_batch = self.__resultToObject(sobject_records)
        return query_result_batch
    
    def __process_count_query_result(self, query_result, callback=None):
        sobject_records, done, query_locator, size = self._unpackQueryResult(query_result)
        if callback is not None:
            callback(size)
        else:
