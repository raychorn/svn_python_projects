...
from libs.utils import is_ip_address_valid, make_number_valid_or_none
...
class ReverseProxy(asyncore.dispatcher):
	...
	try:
	    ...
	    print >>sys.stderr, '%s --> %s:%s' % (ex,ip,port)
	    raise socket.error
        self.listen(backlog)
...
if (__name__ == '__main__'):
    ...
    if (_remote is None) and (options.remote is not None):
	'''
	This pattern is useful for determining if a target end-point is valid in terms of the address for such an end-point.
	'''
	from vyperlogix.sockets.traceroute import TraceRoute
	t = TraceRoute(options.remote,isDebugging=True)
