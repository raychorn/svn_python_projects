...
class ThreadsConnector:
  ...
  def wrap_calc(self, target, args, kw):
    ...
    try:
      ...
      try:
        log.info('INFO - Issuing callback for termination clean-up.')
        self.callback(self.data)
      except Exception, details:
        log.info('WARNING - Cannot issue callback upon thread termination. (%s) (%s)' % (str(details), str(self.callback)))
    except:
      # All other exception
