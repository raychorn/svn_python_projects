...
__ip__ = getupnetips.get_local_ip_address()

class Tracer:

    def __init__(self, host):
        self.id = (os.getpid() & 0xffff) | 0x8000
        self.init_probe_packet(__ip__, host)
        self.times = {}
        self.got_there = None
        # control be
...
class CmdlineTracer(Tracer):
    ...
    def init(self):
        if (self.__host__ is None) and (self.host is not None):
            self.__init__(host=self.host)
...
def main():
    ...
    if len(host) != 1:
        usage()
        return
    t.host = host[0] if (misc.isList(host)) else host
    t.init()
    t.open_sockets()
    t.trace()
