...
class RibbonButtonBarButtonBase(object):
    ...
    def Check(self, check=True):
        ...
        self.checked = check
        self.bitmap_large = (check and [self.bitmap_large_checked] or [self.bitmap_large_normal])[0]
        self.bitmap_small = (check and [self.bitmap_small_checked] or [self.bitmap_small_normal])[0]
...
class RibbonButtonBar(RibbonControl):
    ...
    def AddCheckButton(self, button_id, label, bitmap_large, bitmap_small, bitmap_large_checked,
                       bitmap_small_checked, help_string=""):

        button = self.AddButton(button_id, label, bitmap_large, bitmap_small, wx.NullBitmap,
                                wx.NullBitmap, RIBBON_BUTTON_NORMAL, help_string)

        button.bitmap_large_checked = bitmap_large_checked
        button.bitmap_small_checked = bitmap_small_checked
        button.bitmap_large_normal = bitmap_large
        button.bitmap_small_normal = bitmap_small

        return button        
        

    def SetButtonBitmaps(self, button_id, bitmap_large, bitmap_small):

        for button in self._buttons:
            if button.id == button_id:
                button.bitmap_small = bitmap_small
                button.bitmap_large = bitmap_large
                return
    ...
    def OnPaint(self, event):
        ...
        self._art.DrawButtonBarBackground(dc, self, wx.RectS(self.GetSize()))
