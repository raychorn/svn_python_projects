...
class TreeModel(object):
    ...
    def SetItems(self, indices, items=[]):
        children = self.GetChildren(indices)
...
class TreeNotebook(wx.Notebook):
    def __init__(self, *args, **kwargs):
        ...
        self.trees = []
        for class_, title in [(VirtualTreeCtrl, titles[0])]:
...
class TreeNotebookPanel(wx.Panel):
    def __init__(self, parent, titles=[], log=None):
        self.log = log
        super(TreeNotebookPanel, self).__init__(parent)
    ...
    def CreateControls(self):
        self.notebook = TreeNotebook(self, treemodel=self.treemodel, log=self.log, titles=self.titles)
    ...
    def addItems(self,items=[]):
        ...
        newChildrenCount = len(items)
        for indices in indicesList:
            self.treemodel.SetItems(indices, items)
        self.notebook.RefreshItems()

    def addItemsTo(self,indicesList=[0],items=[]):
        self.treemodel.SetItems(indicesList, items)
        self.notebook.RefreshItems()
